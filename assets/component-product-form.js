  ...
  _gwpFeature(){
    fetch('/cart.js').then(response => {
      return response.json();
        ...
        console.log(typeof(parseInt(gwp_cart_total)));
        let cart_total_Price = response.items_subtotal_price;
        console.log(typeof(response.total_price));
        if( cart_total_Price >=  parseInt(gwp_cart_total))
        {
          console.log("matched");
          let data= {
            updates:{
...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange(_event) {
    ...
    if (!this.currentVariant) {
      ...
      // console.log(this.currentVariant.store_availability.pick_up_time);
      // this.currentVariant.store_availability.forEach((pickup) => {
      //   console(pickup.location);
      // })
    }
